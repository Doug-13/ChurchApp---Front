...
function toCount(value) {
  ...
  if (typeof value === "object") {
    ...
    return toCount(
      value.count ?? value.total ?? value.totalCount ??
      value.quantity ?? value.length ?? 0
    );
  }
  return 0;
}
...
function normalizeDashboardData({ dashboardData = {}, membersData = [], cellsData = [], eventsData = [] } = {}) {
  return {
    members: toCount(
      membersData?.members ?? membersData?.data ?? membersData?.items ??
      membersData?.results ?? membersData ??
      dashboardData?.members ?? dashboardData?.totalMembers ??
      dashboardData?.membersCount ?? dashboardData?.churchMembers
    ),
    cells: toCount(
      cellsData?.cells ?? cellsData?.data ?? cellsData?.items ??
      cellsData?.results ?? cellsData ??
      dashboardData?.cells ?? dashboardData?.totalCells ?? dashboardData?.cellsCount
    ),
    eventsPending: toCount(
      eventsData?.events ?? eventsData?.data ?? eventsData?.items ??
      eventsData?.results ?? eventsData ??
      dashboardData?.eventsPending ?? dashboardData?.pendingEvents ??
      dashboardData?.events ?? dashboardData?.eventsCount
    ),
    approvals: toCount(
      dashboardData?.approvals ?? dashboardData?.pendingApprovals ??
      dashboardData?.approvalsCount ?? dashboardData?.requestsPending
    ),
  };
}
  ...
  return (
    <View style={styles.sectionHeader}>
      ...
      <View style={{ flex: 1 }}>
        ...
        {!!subtitle && (
          <Text style={[styles.sectionSubtitle, { color: tc.muted }]}>{subtitle}</Text>
        )}
      </View>
      {!!actionLabel && (
        <TouchableRipple onPress={onAction} borderless style={styles.sectionAction}>
          <Text style={[styles.sectionActionText, { color: BRAND_BLUE }]}>{actionLabel}</Text>
        </TouchableRipple>
      )}
    </View>
  );
  ...
  return (
    <Surface elevation={1} style={[styles.metricCard, { backgroundColor: tc.surface, borderColor: tc.outline }]}>
      <View style={styles.metricTop}>
        ...
        <Text style={[styles.metricLabel, { color: tc.muted }]} numberOfLines={1}>{label}</Text>
      </View>
      <Text style={styles.metricValue}>{formatNumber(value)}</Text>
      {!!helper && (
        <Text style={[styles.metricHelper, { color: tc.muted }]} numberOfLines={1}>{helper}</Text>
      )}
    </Surface>
  );
}
...
export default function AdminDashboardScreen({ navigation }) {
  ...
  const [stats,        setStats]        = useState({ members: 0, cells: 0, eventsPending: 0, approvals: 0 });
  const [loading,      setLoading]      = useState(true);
  const [refreshing,   setRefreshing]   = useState(false);
  const [errorMessage, setErrorMessage] = useState("");

  const perms    = getPermissions(myRole);
  const roleMeta = ROLE_META[normalizeRole(myRole)] ?? ROLE_META.MEMBER;
  ...
  const loadDashboard = useCallback(async ({ showRefresh = false } = {}) => {
    try {
      ...
      const [mineData, dashboardData, membersData, cellsData, eventsData] =
        await Promise.all([
          apiFetchAuth("/churches/mine", { method: "GET" }).catch(() => null),
          apiFetchAuth("/users/me/dashboard", { method: "GET" }).catch(() => ({})),
          apiFetchAuth("/users/members", { method: "GET" }).catch(() => []),
          cellsPath  ? apiFetchAuth(cellsPath,  { method: "GET" }).catch(() => []) : Promise.resolve([]),
          eventsPath ? apiFetchAuth(eventsPath, { method: "GET" }).catch(() => []) : Promise.resolve([]),
        ]);

      const mine     = Array.isArray(mineData) ? mineData : (mineData ? [mineData] : []);
      const selected = (activeChurchId && mine.find((c) => c.id === activeChurchId)) || mine[0] || null;
      const roleRaw  = selected?.myRole || selected?.role || null;
      setMyRole(roleRaw);

      setStats(normalizeDashboardData({ dashboardData, membersData, cellsData, eventsData }));
    } catch (error) {
      setErrorMessage(error?.message || "Não foi possível carregar os dados do painel.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
      ...
      >
        ...
        {!!errorMessage && (
          <View style={styles.errorCard}>
            ...
            <TouchableRipple onPress={() => loadDashboard()} borderless style={styles.errorBtn}>
        ...
        <View style={styles.hero}>
          ...
          <View style={styles.heroTop}>
            <View style={styles.heroIconWrap}>
              <Icon source="church" size={20} color="#fff" />
            </View>
            {perms.canViewReports && (
              <TouchableRipple onPress={() => navigation.navigate("Reports")} borderless>
                <View style={styles.heroReportBtn}>
                  <Icon source="chart-box-outline" size={13} color="#fff" />
        ...
        <View style={styles.shortcutsGrid}>
          ...
          {perms.canViewReports && (
            <ShortcutCard
              ...
              icon="chart-box-outline" color={BRAND_BLUE} bg={BRAND_LIGHT} tc={tc}
          ...
          {perms.canManageChurchProfile && (
            <ShortcutCard
              ...
              icon="church" color={NAVY} bg={BRAND_LIGHT} tc={tc}
        ...
        <View style={styles.metricsGrid}>
          ...
          <MetricCard
            label="Eventos" value={stats.eventsPending}
            icon="calendar-alert" helper="pendentes"
            color={WARNING} bg={WARNING_BG} tc={tc}
          />
          {perms.canApproveMember && (
            <MetricCard
              label="Aprovações" value={stats.approvals}
              icon="account-check-outline" helper="aguardando"
        ...
        <View style={styles.pendingList}>
          ...
          {!perms.canCreateEvent && !perms.canApproveMember && (
            <PendingItem
        ...
        {perms.canCreateEvent && (
          <View style={styles.ctaCard}>
            <View style={[styles.blob, { width: 160, height: 160, top: -40, right: -30 }]} />
            <View style={styles.ctaContent}>
              <View style={styles.ctaTop}>
                <View style={styles.ctaIconWrap}>
                  <Icon source="calendar-plus" size={20} color="#fff" />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.ctaTitle}>
                    {perms.canManageEventScales ? "Organize o próximo evento" : "Crie um novo evento"}
                  </Text>
