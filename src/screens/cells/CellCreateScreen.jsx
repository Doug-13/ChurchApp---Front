...
function getCellLeader(cell) {
  return normalizeUserOption(
    ...
    (cell?.leaderUserId
      ? {
        id: cell.leaderUserId,
        fullName: cell?.leaderName || cell?.leaderUserName || cell?.leaderFullName || "Líder",
      }
      : null)
  );
}
...
function getCellViceLeader(cell) {
  return normalizeUserOption(
    cell?.viceLeader ||
    cell?.viceLeaderUser ||
    (cell?.viceLeaderUserId
      ? {
        id: cell.viceLeaderUserId,
        fullName: cell?.viceLeaderName || cell?.viceLeaderUserName || cell?.viceLeaderFullName || "Vice-líder",
      }
      : null)
  );
}
  ...
  return (
    <TouchableRipple
      ...
      style={[
        styles.selectChip,
        {
          backgroundColor: active ? withAlpha(color, "18") : BG,
  ...
  return (
    <View style={{ gap: 6 }}>
      ...
      <TouchableRipple
        ...
        disabled={disabled}
        borderless
        style={[styles.selectField, disabled && { opacity: 0.55 }]}
      >
        <View style={{ flexDirection: "row", alignItems: "center", gap: 12, flex: 1 }}>
          ...
          <Text
            style={[
              ...
              {
                color: hasValue ? NAVY : MUTED,
                fontWeight: hasValue ? "900" : "600",
              },
            ]}
            numberOfLines={1}
          >
              ...
              <TouchableRipple
                ...
                style={[
                  styles.cascadeRow,
                  {
                    ...
                    backgroundColor: active ? BRAND_LIGHT : SURFACE,
                  },
                ]}
              >
                <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
                  {isHttpUrl(u.photoUrl) ? (
                    ...
                    <Avatar.Text
                      ...
                      color="#fff"
                      style={{ backgroundColor: active ? BRAND : MUTED }}
                    />
                  )}

                  <View style={{ flex: 1 }}>
                    <Text style={[styles.memberName, { color: active ? BRAND : NAVY }]} numberOfLines={1}>
                      {u.fullName}
                    </Text>
...
function InfoPill({ icon, label }) {
  return (
    <View style={[styles.infoPill, { backgroundColor: BRAND_LIGHT }]}>
      ...
      <Text style={[styles.infoPillText, { color: NAVY }]} numberOfLines={1}>
        {label}
      </Text>
    </View>
  );
...
function ColorOption({ color, name, selected, onPress }) {
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.colorOption,
        {
          ...
          backgroundColor: selected ? withAlpha(color, "16") : SURFACE,
    ...
    >
      ...
      <Text style={[styles.colorOptionText, { color: selected ? color : NAVY }]} numberOfLines={1}>
        {name}
      </Text>
    </Pressable>
  );
...
export default function CellCreateScreen({ navigation, route }) {
  ...
  useEffect(() => {
    if (routeCell) {
      applyCellToForm(routeCell);
    }
  }, [routeCell, applyCellToForm]);
  ...
  const loadCellForEdit = useCallback(async () => {
    ...
    try {
      console.log("🟦 [CellCreateScreen] GET célula para edição:", editCellId);

      const data = await authedFetch(ENDPOINTS.getCell(editCellId), {}, authCtx);

      console.log("🟩 [CellCreateScreen] célula carregada para edição:", data);

      setLoadedCell(data);
      applyCellToForm(data);
    } catch (e) {
      console.log("🟥 [CellCreateScreen] erro ao carregar célula para edição:", {
        status: e?.status,
        message: e?.message,
        payload: e?.payload,
      });

      setError(e?.message || "Erro ao carregar célula para edição.");
    } finally {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      const normalized = arr
        .map((r) => ({
          id: r?.id,
          fullName: r?.fullName || r?.name || r?.displayName || "Membro",
          email: r?.email || null,
          phone: r?.phone || null,
      ...
      console.log("🟨 [CellCreateScreen] erro ao carregar usuários:", {
        code: err?.code,
        status: err?.status,
        message: err?.message,
      });
  ...
  const filterUsers = useCallback((list, q) => {
    ...
    return list.filter((u) =>
      [u.fullName, u.email, u.phone].filter(Boolean).join(" ").toLowerCase().includes(term)
    );
  }, []);
  ...
  const handleCoverImagePick = useCallback(() => {
    ImagePicker.openPicker({
      ...
      .then((image) => {
        console.log("🟦 [CellCreateScreen] imagem da célula selecionada:", {
          path: image?.path,
          mime: image?.mime,
          size: image?.size,
          width: image?.width,
      ...
      .catch((err) => {
        console.log("🟨 [CellCreateScreen] seleção cancelada ou falhou:", {
          code: err?.code,
          message: err?.message,
        });

        if (err?.code !== "E_PICKER_CANCELLED") {
          Alert.alert("Erro", "Não foi possível selecionar a imagem da célula.");
        }
      });
  }, []);

  const uploadCellCoverIfNeeded = useCallback(
    async (localPathOrUrl) => {
      if (!localPathOrUrl) return null;

      if (/^https?:\/\//i.test(localPathOrUrl)) {
        console.log("ℹ️ [Cell Storage] imagem já é URL, upload ignorado:", localPathOrUrl);
        return localPathOrUrl;
      }

      if (!churchId) {
        throw new Error("Nenhuma igreja ativa encontrada para salvar a imagem da célula.");
      }
      ...
      console.log("🟦 [Cell Storage] preparando upload da imagem da célula");
      console.log("🟦 [Cell Storage] churchId:", churchId);
      console.log("🟦 [Cell Storage] uploadUri:", uploadUri);
      console.log("🟦 [Cell Storage] storagePath:", storagePath);
      ...
      console.log("🟩 [Cell Storage] imagem da célula enviada:", downloadUrl);
  ...
  const onSave = useCallback(async () => {
    ...
    try {
      ...
      if (selectedCover) {
        finalPhotoUrl = await uploadCellCoverIfNeeded(selectedCover);
      }
      ...
      console.log(
        isEditMode
          ? "🟦 [CellCreateScreen] PATCH célula payload:"
          : "🟦 [CellCreateScreen] POST célula payload:",
        payload
      );

      if (isEditMode) {
        await authedFetch(
          `/cells/${encodeURIComponent(editCellId)}`,
          {
            method: "PATCH",
            body: payload,
          },
        ...
        setSnack({ visible: true, text: "Célula atualizada com sucesso!" });
      } else {
        await authedFetch(
      ...
      console.log(
        isEditMode
          ? "🟥 [CellCreateScreen] erro ao editar célula:"
          : "🟥 [CellCreateScreen] erro ao criar célula:",
        {
          code: e?.code,
          status: e?.status,
          message: e?.message,
      ...
      setError(e?.message || (isEditMode ? "Erro ao editar célula." : "Erro ao criar célula."));
    } finally {
      setSaving(false);
    }
  }, [
    ...
    <View style={styles.container}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        ...
        >
          ...
          <Surface elevation={0} style={[styles.heroCard, { backgroundColor: NAVY }]}>
            ...
            <View style={styles.heroTop}>
              <Pressable
                ...
                style={({ pressed }) => [styles.heroAvatarPressable, pressed && !saving && { opacity: 0.88 }]}
              >
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.heroGreet}>{isEditMode ? "Editar" : "Criar nova"}</Text>
                <Text style={styles.heroTitle}>{safeStr(name) || "Célula"}</Text>
            ...
            <View style={styles.heroPills}>
              <View style={styles.heroPill}>
                ...
                <Text style={styles.heroPillText}>{isEditMode ? "Modo edição" : "Nova célula"}</Text>
              </View>
              ...
              <View style={styles.heroPill}>
                ...
                <Text style={styles.heroPillText}>Cor da célula</Text>
              </View>
          ...
          {!!error && (
            <Surface elevation={0} style={styles.errorBanner}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.errorTitle}>{isEditMode ? "Não foi possível editar" : "Não foi possível salvar"}</Text>
                <Text style={[styles.mutedText, { marginTop: 2 }]}>{error}</Text>
              </View>
            </Surface>
          )}

          {/* ── Aparência / imagem clicável ───────────────────────────────── */}
          <Surface elevation={0} style={[styles.previewCard, { borderColor: withAlpha(accent, "55") }]}>
            ...
            <View style={{ padding: 14, gap: 14 }}>
              <SectionHeader
                title="Aparência da célula"
                subtitle="Toque na foto para abrir a galeria. Escolha também a cor usada nos cards da célula."
              />
              ...
              <View style={styles.appearanceRow}>
                ...
                <View style={{ flex: 1, gap: 8 }}>
                  <Text style={styles.previewName} numberOfLines={1}>
                    {safeStr(name) || "Nome da célula"}
                  </Text>

                  <Text style={styles.mutedText}>
                    A foto será enviada ao Firebase Storage somente quando você tocar em “{isEditMode ? "Salvar alterações" : "Criar célula"}”.
                  </Text>
                  ...
                  {hasPhoto && (
                    <Button
                      ...
                      disabled={saving}
                      textColor={DANGER}
                      style={{ alignSelf: "flex-start", borderRadius: 999, borderColor: DANGER }}
                    >
                      Remover foto
                    </Button>
                  )}
              ...
              <View style={{ gap: 10 }}>
                <Text style={styles.fieldLabel}>Cor da célula</Text>
                <View style={styles.colorOptionsGrid}>
                  {TEMPLATE_COLORS.map((c) => (
                    <ColorOption
                      ...
                      color={c.hex}
                      name={c.name}
                      selected={templateColor === c.hex}
                      onPress={() => setTemplateColor(c.hex)}
                    />
                  ))}
                </View>
              </View>
              ...
              <View style={styles.previewPills}>
                ...
                {!!leader?.fullName && <InfoPill icon="account-star" label={`Líder: ${leader.fullName}`} />}
          ...
          <Surface elevation={0} style={styles.formCard}>
            ...
            <View style={styles.formBody}>
              <SectionHeader title="Informações" subtitle="Nome e descrição da célula." />
          ...
          <Surface elevation={0} style={styles.formCard}>
            ...
            <View style={styles.formBody}>
              <SectionHeader title="Reunião" subtitle="Você pode definir agora ou depois." />
              ...
              <View style={styles.chipRow}>
                {DAYS.map((d) => (
                  <SelectChip
                    ...
                    label={d.slice(0, 3)}
                    active={meetingDay === d}
                    onPress={() => setMeetingDay(meetingDay === d ? "" : d)}
                    activeColor={WARNING}
                  />
                ))}
              </View>
          ...
          <Surface elevation={0} style={styles.formCard}>
            ...
            <View style={styles.formBody}>
              <SectionHeader title="Liderança" subtitle="Defina o líder e vice-líder da célula." />

              <SelectField
                label="Líder"
                value={leader?.fullName || ""}
                placeholder={!churchId ? "Nenhuma igreja ativa" : usersLoading ? "Carregando..." : "Selecionar membro"}
              ...
              {!!leader?.id && (
                <TouchableRipple
                  ...
                  borderless
                >
                  <Text style={styles.clearBtn}>Limpar líder</Text>
                </TouchableRipple>
              )}
              ...
              {!!viceLeader?.id && (
                <TouchableRipple
                  ...
                  borderless
                >
                  <Text style={styles.clearBtn}>Limpar vice-líder</Text>
                </TouchableRipple>
              )}
          ...
          <Surface elevation={0} style={styles.formCard}>
            <View style={[styles.formStrip, { backgroundColor: accent }]} />
            <View style={styles.formBody}>
              <SectionHeader title="Galeria" subtitle="Opcional. Adicione URLs de imagens extras da célula." />
        ...
        <View style={styles.footer}>
          ...
          >
            {saving ? "Salvando..." : isEditMode ? "Salvar alterações" : "Criar célula"}
          </Button>
        </View>
      </KeyboardAvoidingView>
